...
geih_sample <- geih_sample |>
  mutate(
    ...
    relab = case_when(
      relab %in% c(6, 7, 8, 9) ~ "6_otro_marginal",
...
p_low <- quantile(geih_sample$y_total_m, 0.01, na.rm = TRUE)   # percentil 1
p_high <- quantile(geih_sample$y_total_m, 0.998, na.rm = TRUE) # percentil 99.8
